...
public class BrowserView {
	...
	public String getHomepage() {
		...
		switch (mPreferences.getInt(PrefConstants.SEARCH, 1)) {
			case 0:
				...
				home = home
						+ mPreferences.getString(PrefConstants.SEARCH_URL,
	...
	public synchronized void initializePreferences(Context context) {
		mPreferences = context.getSharedPreferences(PrefConstants.PREFERENCES, 0);
		...
		setColorMode(mPreferences.getInt(PrefConstants.RENDERING_MODE, 0));

		mSettings.setGeolocationEnabled(mPreferences
				.getBoolean(PrefConstants.LOCATION, false));
		if (API < 19) {
			switch (mPreferences.getInt(PrefConstants.ADOBE_FLASH_SUPPORT, 0)) {
				case 0:
		...
		switch (mPreferences.getInt(PrefConstants.USER_AGENT, 1)) {
			case 1:
				if (API > 16) {
			...
			case 4:
				mSettings.setUserAgentString(mPreferences.getString(
						PrefConstants.USER_AGENT_STRING, mDefaultUserAgent));
				break;
		}

		if (mPreferences.getBoolean(PrefConstants.SAVE_PASSWORDS, false)) {
			if (API < 18) {
		...
		if (mPreferences.getBoolean(PrefConstants.JAVASCRIPT, true)) {
			mSettings.setJavaScriptEnabled(true);
			mSettings.setJavaScriptCanOpenWindowsAutomatically(true);
		}

		if (mPreferences.getBoolean(PrefConstants.TEXT_REFLOW, false)) {
		...
		mSettings.setBlockNetworkImage(mPreferences.getBoolean(PrefConstants.BLOCK_IMAGES,
				false));
		mSettings.setSupportMultipleWindows(mPreferences.getBoolean(PrefConstants.POPUPS,
				true));
		mSettings.setUseWideViewPort(mPreferences.getBoolean(PrefConstants.USE_WIDE_VIEWPORT,
				true));
		mSettings.setLoadWithOverviewMode(mPreferences.getBoolean(
				PrefConstants.OVERVIEW_MODE, true));
		switch (mPreferences.getInt(PrefConstants.TEXT_SIZE, 3)) {
			case 1:
