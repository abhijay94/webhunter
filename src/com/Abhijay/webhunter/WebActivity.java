...
public class WebActivity extends Activity implements BrowserController {
	...
	private List<BrowserView> mWebViews = new ArrayList<BrowserView>();
	...
	private synchronized void initialize() {
		...
		if (mWebViews != null) {
			...
			mWebViews = new ArrayList<BrowserView>();
		}
		mBookmarkManager = new BookmarkManager(this);
		if (!mPreferences.getBoolean(PrefConstants.OLD_BOOKMARKS_IMPORTED, false)) {
			List<HistoryItem> old = Utils.getOldBookmarks(this);
			mBookmarkManager.addBookmarkList(old);
			mEditPrefs.putBoolean(PrefConstants.OLD_BOOKMARKS_IMPORTED, true).apply();
		}
		mActivity = this;
		...
		mNewTab.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				String url = mPreferences.getString(PrefConstants.SAVE_URL, null);
				if (url != null) {
					newTab(url, true);
					Toast.makeText(mContext, R.string.deleted_tab, Toast.LENGTH_SHORT).show();
				}
				mEditPrefs.putString(PrefConstants.SAVE_URL, null).apply();
				return true;
			}
	...
	public void restoreOrNewTab() {
		...
		if (mPreferences.getBoolean(PrefConstants.RESTORE_LOST_TABS, true)) {
			String mem = mPreferences.getString(PrefConstants.URL_MEMORY, "");
			mEditPrefs.putString(PrefConstants.URL_MEMORY, "");
	...
	public void initializePreferences() {
		...
		mFullScreen = mPreferences.getBoolean(PrefConstants.FULL_SCREEN, false);
		...
		switch (mPreferences.getInt(PrefConstants.SEARCH, 1)) {
			case 0:
				mSearchText = mPreferences.getString(PrefConstants.SEARCH_URL,
	...
	protected synchronized void newTab(String url, boolean show) {
		mIsNewIntent = false;
		BrowserView startingTab = new BrowserView(mActivity, url);
		if (mIdGenerator == 0) {
	...
	private synchronized void deleteTab(int position) {
		...
		BrowserView reference = mWebViews.get(position);
		if (reference == null) {
			return;
		}
		if (reference.getUrl() != null && !reference.getUrl().startsWith(Constants.FILE)) {
			mEditPrefs.putString(PrefConstants.SAVE_URL, reference.getUrl()).apply();
		}
		...
		if (current > position) {
			...
			if (mCurrentView.getUrl() == null || mCurrentView.getUrl().startsWith(Constants.FILE)
				...
				mIdList.remove(position);
				mWebViews.remove(position);
				if (mPreferences.getBoolean(PrefConstants.CLEAR_CACHE_EXIT, false)
						&& mCurrentView != null && !isIncognito()) {
					mCurrentView.clearCache(true);
					Log.i(Constants.TAG, "Cache Cleared");

				}
				if (mPreferences.getBoolean(PrefConstants.CLEAR_HISTORY_EXIT, false)
						&& !isIncognito()) {
					clearHistory();
					Log.i(Constants.TAG, "History Cleared");

				}
				if (mPreferences.getBoolean(PrefConstants.CLEAR_COOKIES_EXIT, false)
						&& !isIncognito()) {
					clearCookies();
	...
	public void saveOpenTabs() {
		if (mPreferences.getBoolean(PrefConstants.RESTORE_LOST_TABS, true)) {
			...
			mEditPrefs.putString(PrefConstants.URL_MEMORY, s);
			mEditPrefs.commit();
		}
	}
	...
	public BitmapDrawable writeOnDrawable(int number) {
		...
		if (mActionBarSizeDp < 50) {
			if (number > 9) {
				paint.setTextSize(mActionBarSize * 9 / 16); // originally
				// 40
				// pixels,
				// 24 dp
			} else {
				paint.setTextSize(mActionBarSize * 27 / 40); // originally 50
				// pixels, 30 dp
			}
		} else {
			paint.setTextSize(mActionBarSize * 9 / 16);
		}
		...
		paintbox.setColor(getResources().getColor(android.R.color.black));
		float cutter = canvas.getWidth()/7;
		canvas.drawRect(cutter, cutter, canvas.getWidth()-cutter, canvas.getHeight()-cutter, paintbox);
		canvas.drawText(text, xPos, yPos, paint);
		return new BitmapDrawable(getResources(), bm);
	}

	public class LightningViewAdapter extends ArrayAdapter<BrowserView> {

		Context context;

		int layoutResourceId;

		List<BrowserView> data = null;

		public LightningViewAdapter(Context context, int layoutResourceId, List<BrowserView> data) {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			BrowserView web = data.get(position);
		...
		Runnable update = new Runnable() {
			...
			public void run() {
				if (isSystemBrowserAvailable()
						&& mPreferences.getBoolean(PrefConstants.SYNC_HISTORY, true)) {
					try {
	...
	public void onHideCustomView() {
		...
		setFullscreen(mPreferences.getBoolean(PrefConstants.HIDE_STATUS_BAR, false));
		FrameLayout decor = (FrameLayout) getWindow().getDecorView();
		if (decor != null) {
